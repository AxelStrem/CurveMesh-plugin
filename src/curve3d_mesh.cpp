...
static float _get_project_texel_size() {
	...
	const Variant value = settings->get_setting("rendering/lightmapping/primitive_meshes/texel_size");
...
static float _get_width_curve_value(const Ref<Curve> &width_curve, float t, float default_value) {
...
void Curve3DMesh::_update_lightmap_size() {
	...
	lightmap_size_hint.x = (int)Math::ceil(Math::max(1.0f, lightmap_length / texel_size) + 2.0f * padding);

	float lightmap_width = width;
	if (width_curve.is_valid()) {
		lightmap_width *= Math::max(width_curve->get_max_value(), width_curve->get_min_value());
	}
	...
	lightmap_size_hint.y = (int)Math::ceil(Math::max(1.0f, lightmap_width / texel_size) + width_padding * padding);
...
Array Curve3DMesh::_create_mesh_array() const {
	...
	if (curve.is_valid() && curve->get_point_count() > 1) {
		...
		if (center_points.size() >= 2) {
			...
			const int radial_segments = (profile == PROFILE_FLAT) ? 1 : segments;
			_generate_edge_vertices(center_points, total_length, radial_segments, uv2_padding, edge_points);
...
void Curve3DMesh::_generate_edge_vertices(LocalVector<CenterPoint> &center_points, real_t total_length, int radial_segments, float uv2_padding, LocalVector<EdgePoint> &edge_points) const {
	...
	const float length_h = (horizontal_total <= 0.0f || total_length <= 0.0f) ? 0.0f : (total_length / horizontal_total);
	const float padding_h = (horizontal_total <= 0.0f) ? 0.0f : (uv2_padding / horizontal_total);

	const float max_width_sample = width_curve.is_valid() ? Math::max(width_curve->get_max_value(), -width_curve->get_min_value()) : 1.0f;
	const float max_width = width * max_width_sample;
	const float length_v = 1.0f / radial_segments;
	const float edge_padding = length_v * ((profile == PROFILE_TUBE) ? 1.0f : (max_width <= 0.0f ? 1.0f : max_width / (max_width + uv2_padding)));
	...
	for (int i = 0; i < point_count; i++) {
		...
		float u = (total_length > 0.0f) ? center_points[i].partial_length / total_length : 0.0f;
		...
		if (!smooth_shaded_corners && center_points[i].corner_point) {
			...
			for (int e = 0; e < edge_count; e++) {
				...
				for (int j = 0; j < radial_segments; j++) {
					...
					Vector3 normal_rotated = (profile == PROFILE_TUBE) ? -edge * normal.cross(tangent) : normal;
					normal_rotated = normal_rotated.rotated(tangent, (float)j * segment_angle);
					point.normal = normal_rotated;
					int index = edge_points.size();
					point.prev_point = index - radial_segments;
					edge_points[point.prev_point].next_point = index;
	...
	for (int j = 0; j < radial_segments; j++) {
		...
		if (!curve->is_closed()) {
			for (int e = 0; e < edge_count; e++) {
				...
				edge_points[last_index - (edge_count - 1 - e) * radial_segments].next_connected = false;
			}
		}
	}
}
...
void Curve3DMesh::_interleave_edge_vertices(LocalVector<EdgePoint> &edge_points, LocalVector<CenterPoint> &center_points, int radial_segments) const {
	...
	for (int j = 0; j < radial_segments; j++) {
		...
		while (point->next_point >= point_index) {
			...
			if (center_points[point->source_index].corner_point || center_points[next_point->source_index].corner_point || point->source_index == next_point->source_index) {
				point = next_point;
				continue;
			}
			remove_point(*point);
				...
				if (next_point->edge == point->edge) {
					...
					if (profile == PROFILE_TUBE) {
						...
						if (top_dir.cross(next_dir).dot(point->normal) < 0.0f) {
							...
							if (top_side.dot(point_side) > 0.0f && top_side.dot(next_side) > 0.0f) {
								point->filter = true;
							}
						}

						if (next_dir.cross(bottom_dir).dot(point->normal) < 0.0f) {
							Vector3 bottom_side = bottom_point->position - center_points[bottom_point->source_index].position;
							Vector3 next_side = next_point->position - center_points[next_point->source_index].position;
							Vector3 point_side = point->position - center_points[point->source_index].position;
							if (bottom_side.dot(point_side) > 0.0f && bottom_side.dot(next_side) > 0.0f) {
								next_point->filter = true;
							}
						}
					}
		...
		for (uint32_t k = 0; k < edge_points.size(); ++k) {
			...
			if (point->filter && !point->removed) {
				...
				if (group_indices.size() > 1) {
					...
					if (before_idx >= 0 && after_idx < (int)edge_points.size() && edge_points[before_idx].edge == point->edge && edge_points[after_idx].edge == point->edge) {
					...
					for (uint32_t i = 1; i < group_indices.size(); ++i) {
						EdgePoint *remove_candidate = &edge_points[group_indices[i]];
...
void Curve3DMesh::_generate_triangles(LocalVector<EdgePoint> &edge_points, int radial_segments, PackedVector3Array &points, PackedVector3Array &normals, PackedFloat32Array &tangents, PackedVector2Array &uvs, PackedVector2Array &uv2s, PackedInt32Array &indices) const {
	const bool add_uv2 = get_add_uv2();
...
void Curve3DMesh::_validate_property(PropertyInfo &p_property) const {
	static const StringName tessellation_tolerance_name("tessellation_tolerance");
	...
	if (property_name == tessellation_tolerance_name) {
		...
		p_property.usage = (profile == PROFILE_CROSS) ? PROPERTY_USAGE_DEFAULT : PROPERTY_USAGE_NO_EDITOR;
	} else if (property_name == interleave_vertices_name) {
		p_property.usage = (profile == PROFILE_TUBE) ? PROPERTY_USAGE_NO_EDITOR : PROPERTY_USAGE_DEFAULT;
	} else if (property_name == scale_uv_by_width_name) {
		p_property.usage = (profile == PROFILE_FLAT) ? PROPERTY_USAGE_DEFAULT : PROPERTY_USAGE_NO_EDITOR;
	}
}
...
void Curve3DMesh::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("set_scale_uv_by_length", "enable"), &Curve3DMesh::set_scale_uv_by_length);
	ClassDB::bind_method(D_METHOD("is_scale_uv_by_length"), &Curve3DMesh::is_scale_uv_by_length);

	ClassDB::bind_method(D_METHOD("set_scale_uv_by_width", "enable"), &Curve3DMesh::set_scale_uv_by_width);
	...
	ClassDB::bind_method(D_METHOD("set_follow_curve", "follow"), &Curve3DMesh::set_follow_curve);
...
void Curve3DMesh::set_curve(const Ref<Curve3D> &p_curve) {
	...
	if (curve.is_valid()) {
		curve->connect("changed", update_callable, Object::CONNECT_REFERENCE_COUNTED);
	}
...
void Curve3DMesh::set_width_curve(const Ref<Curve> &p_curve) {
	...
	if (width_curve.is_valid()) {
		width_curve->connect("changed", update_callable, Object::CONNECT_REFERENCE_COUNTED);
	}
